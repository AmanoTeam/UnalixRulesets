/*
This program parses the file at unalix.json to check if it complies with the ClearURLs rule catalog specification.
*/
...
static const char* const BOOLEAN_KEYS[] = {
	(const char* const) COMPLETE_PROVIDER,
	(const char* const) FORCE_REDIRECTION
};

static const char* const ARRAY_KEYS[] = {
	(const char* const) RULES,
	(const char* const) RAW_RULES,
	(const char* const) REFERRAL_MARKETING,
	(const char* const) EXCEPTIONS,
	(const char* const) REDIRECTIONS
};
...
int main() {
	...
	tree = json_load_file(RULESET_FILE, 0, &error);
